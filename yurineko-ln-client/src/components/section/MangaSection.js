...
export default function MangaSection({
  ...
  const handleChangePage = (newPage) => {
    if (newPage != 1) {
      route.push(
        `${route.route}?page=${newPage}${
          Object.keys(route.query).length >= 1 && !route.query.page ? '&' : ''
      ...
      <div className="relative">
        <div className="px-1 md:px-0 py-3">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-pink darK:text-dark-text">
              {icon && <span className="mr-1">{icon}</span>}
              <span className="text-2xl font-bold dark:text-dark-text">{title}</span>
            </h2>
            <ViewMode handleChangeView={handleChangeView} isGrid={isGrid} />
          </div>
          {isLoading == true || _.isEmpty(data[page]) ? (
              ...
              {isGrid == true ? (
                  ...
                  <Row className="p-2" gutter={{ xs: 8, sm: 16, md: 20 }}>
                    {data[page].result.map((item) => (
                      <Col xs={12} sm={8} xxl={6} key={`grid-${item.id}`}>
                        <MangaBoxGrid manga={item} list={list} />
                      </Col>
                    ))}
                  </Row>
                </div>
                ...
                <div className="mt-2">
                  {isSearch == false ? (
                    <Row>
                      {data[page].result.map((item) => (
                        <Col className="mb-2" xs={24} key={`list-${item.id}`}>
                          <MangaBoxList manga={item} list={list} />
                        </Col>
                      ))}
                    </Row>
                  ) : (
                    ...
                    >
                      {data[page].result.map((item) => (
                        <Col xs={24} md={12} key={`list-search-${item.id}`}>
                          <MangaBoxList manga={item} />
                        </Col>
                      ))}
                    </Row>
                  )}
