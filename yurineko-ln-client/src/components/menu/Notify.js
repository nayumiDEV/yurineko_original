...
function Notify({ notification, ...props }) {
  ...
  const fetchMoreNotif = () => {
    props.getNotification(notification.currentPage + 1);
  }
        ...
        {notification && (
          ...
          >
            {notification.data.map((item) => (
              <div className={`cursor-pointer border-0 py-2.5 px-4 dark:text-dark-text ${item.isRead == false ? "bg-pink-lightness dark:bg-dark-cyan" : ""}`}>
                <a href={item.url ?? '#'} onClick={() => readNotify(item.id).then(() => true).catch(() => false)}>
                  <div className="flex flex-row space-x-3">
                    <div className="flex-shrink-0">
                      <div className="relative">
                        <img
                          alt="Yurineko"
                          src={`${BASE_STORAGE}/${item.thumbnail}`}
                          className="w-16 h-16 rounded-full object-cover object-center"
                        >
                        </img>
                        {iconRouter(item.icon || 'reaction-heart')}
                      </div>
                    </div>
                    <div className="flex flex-col">
                      <span className={`line-clamp-2 overflow-ellipsis ${item.isRead == false ? "text-black dark:text-white" : "text-notif-read dark:text-notif-read-dark"}`} style={{ fontSize: "1.1rem" }} dangerouslySetInnerHTML={{ __html: item.title }}></span>
                      <span className="text-sm text-notif-read dark:text-notif-read-dark">{timeFromNow(item.createAt)}</span>
                    </div>
                  </div>
                </a>
              </div>
