import { getBanner, getManga } from 'api/general'
import React, { useEffect, useState } from 'react'
import { Pagination, Row, Col } from 'antd'
import LayoutHome from '@/layouts/Home'
import Link from 'next/link'
import Banner from '@/components/banner/Banner'
import NormalTag from '@/components/tag/NormalTag'
...
import { Description, TeamName } from '@/components/tag/Text'
...
import { authorLink, originLink, lightnovelReadLink, teamLink } from '@/utils/generateLink'
...
import CommentAds from '@/components/ads/CommentAds'
...
function manga({ banner, manga, id, ...props }) {
                  ...
                  <Col xs={24} md={7}>
                    <div className="md:m-0 m-2 mx-auto w-full flex items-center justify-center rounded-xl shadow-lg overflow-hidden">
                      <img
                        ...
                        src={"https://storage.yurineko.moe/" + imgLink(manga.thumbnail)}
                        alt={manga.originalName}
                      />
                    </div>
                  </Col>
                  <Col xs={24} md={17}>
                    <div className="p-2">
                      ...
                      <p className="text-md font-normal md:text-xl">
                        <span className="font-semibold mr-2">Tác giả/Họa sĩ:</span>
                      ...
                      <p className="text-md font-normal md:text-xl">
                        ...
                        <span style={{ fontSize: '1.1rem' }} className="text-button-manga ">
  ...
  try {
    ...
    if (id) {
      ...
      const res = await axios({
        method: 'get',
        url: `${process.env.BASE_URL_LN}/lightnovel/${id}`,
        headers: header,
      })
      return { manga: res.data, id }
    }
  } catch (err) {

    return { manga: false, id }
  }
}
