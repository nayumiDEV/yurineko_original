...
import { getDonateList, delData, addData, editData } from "../../redux/actions";
...
import DataField from "../table/AdminDonate";
...
function TabAdminListManga({ ...props }) {
  ...
  const handleDelete = (id) => {
    props.delData({ type: "donate", id }, getData);
  };

  const handleAdd = (data) => {
    props.addData({ type: "donte", data }, getData);
  };

  const handleEdit = (data) => {
    let newData = { ...editData, ...data };
    props.editData({ type: "donate", data: newData }, getData);
    handleCloseDrawer();
  };
