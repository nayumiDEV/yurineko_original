...
import { getNovelList } from "api/admin";
...
export default function AdminOriginal({
  ...
  const onSearch = async (text) => {
    if (text.length > 0) {
      const res = await getNovelList({query: text});
      setSearchData(mapFromResultToData(res.result));
    } else setSearchData([]);
  };
  ...
  const columns = [
    {
      ...
      key: "originalName",
      render: (text, item) => (
        <a target="_blank" href={process.env.REACT_APP_LN_URL + `/novel/${item.key}`}>
          {text}
        </a>
      ),
    },
    ...
    },
    {
      ...
      render: (team, item) => (
        <a
          target="_blank"
          href={process.env.REACT_APP_LN_URL + `/team/${item.teamUrl}`}
        >
          {item.teamName}
        </a>
      ),
