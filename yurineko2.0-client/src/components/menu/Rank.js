...
import { getR18Ranking, getRanking } from 'api/general'
...
function Rank({ isR18 = false, ...props }) {
  ...
  useEffect(async () => {
    setIsLoading(true)
    if (isR18) {
      getR18Ranking(type[selectedType].type)
        .then((res) => {
          setRank(res)
          setIsLoading(false)
        })
      ...
      getRanking(type[selectedType].type)
        .then((res) => {
          setRank(res)
          setIsLoading(false)
        })
        ...
        <div className="transform -translate-y-2 bg-gray-200 dark:bg-dark-gray pt-4">
          {data &&
            data.map((item, index) => {
              return (
                <>
                  <MangaRank item={item} index={index} key={`manga-${index}`} type={selectedType} />
