              ...
              {isLoading == true ? (
                ...
                <ul className="w-full">
                  {result.slice(0, 6).map((item) => (
                    <a key={item.id} href={mangaLink(item.id)}>
                      <li className="flex my-1 cursor-pointer hover:bg-blue-light w-full overflow-hidden items-center">
                        ...
                        <div className="pl-2 flex-1">
                          ...
                          <p className="text-blue font-semibold leading-tight text-xs italic clamp-1">
                            {item.author?.map((item) => <a>{item.name}</a>) ?? ''}
                          </p>
                          <div className="clamp-1">
                            {item.tag?.map((item) => (
                              <span className="mr-1 italic text-xs text-gray leading-tight">
                                {item.name}
                              </span>
                            )) ?? ''}
                            {item.couple?.map((item) => (
                              <span className="mr-1 italic text-xs text-pink leading-tight">
                                {item.name}
                              </span>
                            )) ?? ''}
                          </div>
