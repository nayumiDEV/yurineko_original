...
function GuestInfo({ user, ...props }) {
  ...
  if (user)
    return (
      <div className="dark:text-dark-text mx-auto">
        <div className="">
          ...
          {user.team && user.team.id != 1 && (
            <Link href={teamLink(user.team.url ? user.team.url : user.team.id)}>
              <p className="font-semibold text-md cursor-pointer">
                ...
                <span className="text-pink-dark">{user.team.name}</span>
              </p>
            </Link>
          )}
        ...
        <div className="p-2 border-t border-gray-300 transform -translate-y-8">
          ...
          <p className="text-seimbold">
            <span className="font-bold">Sở thích:</span> <span>{user.love}</span>
          </p>
          <div>
            <p className="text-seimbold">
