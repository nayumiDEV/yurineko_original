...
export const loginBanned = createAction(constants.ACCOUNT_BANNED)
...
export const getNotification = (cursor = "") => {
  return async (dispatch) => {
    api
      .getNotify(cursor)
      .then((res) => {
        dispatch(getNotificationSuccess({ data: res }))
      })
      .catch((err) => {
        dispatch(handleErr(err))
      })
