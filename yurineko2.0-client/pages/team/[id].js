...
import { getTeam, searchByType, searchLnByType } from 'api/general'
...
function show({ ...props }) {
  ...
  const [apiType, setApiType] = useState('manga');
    ...
    if (_.isEmpty(data[apiType]) || _.isEmpty(data[apiType][page])) {
      ...
      try {
        ...
        if (apiType == 'manga') {
          ...
          res = await searchLnByType({ type, id }, page)
        }
        ...
        <div className="container mx-auto xl:px-40">
          ...
          <Row className="mx-auto" gutter={[20, 16]}>
            <Col xs={24} xl={16} className="py-2">
              <DynamicSection
                selector={
                  <Select className="w-40" defaultValue={"manga"} onChange={(value) => setApiType(value)}>
